...
function GroupDetails() {
  ...
  // Query group details, members, creator, and transactions
  ...
  const groupTransactions = useQuery(api.groups.getGroupTransactions, { groupId: id }) || [];
  ...
  const handleInvite = async () => {
    try {
      const invite = await createInvite({ group_id: groupDetails._id as Id<"groups"> });
    ...
    <div className="p-1 overflow-y-auto max-h-screen">
      <div className="pb-16">
        <div className="sticky top-0 z-50 bg-background shadow-md">
          <div className="text-white p-14 bg-background rounded-lg shadow-md" style={{
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-center flex-grow">
                ...
                <p className="text-lg font-bold text-gray-300 dark:text-gray-400">{groupDetails.description}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-2">
          <div className="rounded-lg pb-2 border shadow-md mt-4">
            <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.4 }} className="text-xl font-bold p-3 pb-1">
              Group Information
            </motion.h2>
            <div className="px-3 py-2">
              <div className="mt-1 space-y-4">
                <div className="rounded-lg bg-card border border-gray-500 shadow p-4">
                  <p><strong>Created By:</strong> {groupDetails.created_by || "Unknown User"}</p>
                  <p><strong>Default Split Type:</strong> {groupDetails.default_split_type}</p>
                  {/* Display Default Split Percentages if applicable */}
                  {groupDetails.default_split_type === "Percentage" && groupDetails.default_split_percentages && (
                    <div className="mt-2">
                      <strong>Default Split Percentages:</strong>
                      <ul className="list-disc list-inside">
                        {groupDetails.default_split_percentages.map((split, index) => {
                          const member = groupMembers.find(member => member._id === split.group_member_id);
                          return <li key={index}>{member?.user?.name || "Unknown Member"}: {split.percentage}%</li>;
                        })}
                      </ul>
                    </div>
                  )}
          ...
          <div className="rounded-lg pb-2 border shadow-md mt-4">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.4 }} className="p-3">
